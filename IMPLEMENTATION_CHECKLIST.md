...
- [ ] Set the `ANTHROPIC_API_KEY` Supabase Edge Function secret (dashboard or `supabase secrets set`, never in chat) — see `docs/ai-setup.md`. Until this is set, every AI feature (Budget Buddy, receipt/pantry scans) fails at the last step (a generic 500) after correctly passing auth and usage-limit checks — verified live against the deployed functions.
...
**Known limitations carried forward:** the central blocker — `ANTHROPIC_API_KEY` is not yet set as an Edge Function secret, so no AI feature can return a real Claude response yet (everything up to that call is built and verified; see `docs/ai-setup.md` for how to set it). No receipts list/detail screen, meal ideas, grocery insights, or Missions library yet (explicit scope cuts, not oversights — see above). Receipt/pantry photos are sent to the Edge Function as base64 for extraction but are never uploaded to the `receipts`/`pantry` Storage buckets created in Phase 2 — only the extracted data is saved, not the source image; wiring up the actual image upload is a small follow-up, not a blocker. No image compression/resizing beyond `expo-image-picker`'s own `quality: 0.7` JPEG setting.
